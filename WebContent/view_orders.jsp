        	...
        	<%
				...
				try {
					switch (acctType){
						...
						case "A":
						case "S":
							sql = "		SELECT `Orders`.`orderID`,`Orders`.`buyerID`,`Orders`.`orderTimestamp`,`Users`.`email`, SUM(`OrderItems`.`quantity`) as itemCount, SUM(  `OrderItems`.`hasShipped` *  `OrderItems`.`quantity` ) AS numItemsShipped, SUM(  `OrderItems`.`canceled` *  `OrderItems`.`quantity` ) AS numItemsCanceled, SUM(`OrderItems`.`unitPrice` * `OrderItems`.`quantity`) as totalCost, SUM(`OrderItems`.`shippingPrice` * `OrderItems`.`quantity`) as totalShipping, SUM(`OrderItems`.`tax` * `OrderItems`.`quantity`) as totalTax" + 
